...
def calculate_metric(preds, labels):
    # 计算每个类别的相关指标：
    # preds 是预测值，labels 是真实标签

    # n_correct_pos：每个类别正确预测的正样本数（preds 和 labels 都为 1 的位置之和）
    n_correct_pos = (labels * preds).sum(0)

    # n_pred_pos：每个类别预测为正样本的总数（preds == 1 的位置之和）
    n_pred_pos = ((preds == 1)).sum(0)

    # n_true_pos：每个类别真实为正样本的总数（labels == 1 的位置之和）
    n_true_pos = labels.sum(0)

    # n_false_pos：每个类别预测为正样本，但真实标签为负的总数
    n_false_pos = ((preds == 1) & (labels == 0)).sum(0)

    # n_false_neg：每个类别预测为负样本，但真实标签为正的总数
    n_false_neg = ((preds == 0) & (labels == 1)).sum(0)

    # 计算 Precision（精度）和 Recall（召回率）：
    precision = n_correct_pos / (n_correct_pos + n_false_pos + np.finfo(np.float64).eps)  # 防止除零
    recall = n_correct_pos / (n_correct_pos + n_false_neg + np.finfo(np.float64).eps)  # 防止除零

    # 计算 OP (Overall Precision)：所有类别的正样本预测正确率的加权平均值
    OP = n_correct_pos.sum() / n_pred_pos.sum()

    # 计算 CP (Class-wise Precision)：每个类别的正样本预测正确率的平均值
    CP = np.nanmean(n_correct_pos / n_pred_pos)

    # 计算 OR (Overall Recall)：所有类别的正样本预测正确率的加权平均值
    OR = n_correct_pos.sum() / n_true_pos.sum()

    # 计算 CR (Class-wise Recall)：每个类别的正样本预测正确率的平均值
    CR = np.nanmean(n_correct_pos / n_true_pos)

    # 计算 CF1 (Class-wise F1 Score)：根据 CP 和 CR 计算每个类别的 F1 分数
    CF1 = (2 * CP * CR) / (CP + CR)

    # 计算 OF1 (Overall F1 Score)：根据 OP 和 OR 计算整体的 F1 分数
    OF1 = (2 * OP * OR) / (OP + OR)

    return CP, CR, CF1, OP, OR, OF1, precision, recall  # 返回 Precision 和 Recall
...
def sl_mAP_cf1_of1(imagessetfilelist, num, pos_ratio):
    ...
    for imagessetfile in imagessetfilelist:
        with open(imagessetfile, 'r') as f:
            lines.extend(f.readlines())  # 读取所有文件内容

    # 解析文件中的数据
    seg = np.array([x.strip().split(' ') for x in lines]).astype(float)
    gt_label = seg[:, num:].astype(np.int32)  # 真实标签
    num_target = np.sum(gt_label, axis=1, keepdims=True)  # 每个样本的目标总数
    threshold = 1 / (num_target + 1e-6)  # 计算阈值（避免除零）

    # 使用阈值进行预测：大于阈值的设为 1，其他设为 0
    predict_result = seg[:, 0:num] > threshold

    # 初始化变量
    sample_num = len(gt_label)
    class_num = num
    tp = np.zeros(sample_num)  # 真阳性
    fp = np.zeros(sample_num)  # 假阳性
    aps = []  # 每个类别的平均精度
    per_class_recall = []  # 每个类别的召回率

    # 针对每个类别计算指标
    for class_id in range(class_num):
        confidence = seg[:, class_id]  # 获取当前类别的预测置信度
        sorted_ind = np.argsort(-confidence)  # 按置信度降序排序
        sorted_scores = np.sort(-confidence)  # 按置信度排序
        sorted_label = [gt_label[x][class_id] for x in sorted_ind]  # 排序后的真实标签

        for i in range(sample_num):
            tp[i] = (sorted_label[i] > 0)  # 如果真实标签为正样本，真阳性
            fp[i] = (sorted_label[i] <= 0)  # 如果真实标签为负样本，假阳性

        true_num = sum(tp)  # 真阳性数量
        fp = np.cumsum(fp)  # 假阳性的累积数量
        tp = np.cumsum(tp)  # 真阳性的累积数量
        rec = tp / float(true_num)  # 计算召回率
        prec = tp / np.maximum(tp + fp, np.finfo(np.float64).eps)  # 计算精度
        ap = voc_ap(rec, prec, true_num)  # 计算该类别的 AP
        aps.append(ap)

    np.set_printoptions(precision=6, suppress=True)  # 设置输出精度
    mAP = np.mean(aps)  # 计算所有类别的平均 mAP

    # 使用 pos_ratio 计算预测标签并计算各类指标
    preds, top_preds = label_decision(seg[:, 0:num], pos_ratio)

    # 计算 CP、CR、CF1、OP、OR、OF1 等指标
    CP, CR, CF1, OP, OR, OF1, precision, recall = calculate_metric(preds, gt_label)
    top_CP, top_CR, top_CF1, top_OP, top_OR, top_OF1, top_precision, top_recall = calculate_metric(top_preds, gt_label)

    # 返回最终结果：mAP、每个类别的 AP、CF1、OF1、Precision 和 Recall
    return mAP * 100\
        , np.array(aps) * 100\
        , [CP * 100, CR * 100, CF1 * 100, OP * 100, OR * 100, OF1 * 100]\
        , [top_CP * 100, top_CR * 100, top_CF1 * 100, top_OP * 100, top_OR * 100, top_OF1 * 100]\
        , [seg[:, 0:num], preds, gt_label]\
        , [precision * 100]\
        , [recall * 100]\
        , [top_precision * 100]\
        , [top_recall * 100]

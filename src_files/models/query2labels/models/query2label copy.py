...
class Qeruy2Label(nn.Module):
    def __init__(self, backbone, transfomer, num_class, label_names, learnable_mask=False):
        # , label_names, clip_model_name,learnable_mask=False
        super().__init__()
    ...
    def build_label_mask(label_names):
        """根据天气标签的语义关系构建掩码矩阵"""
        # 示例关系：允许关注的标签对（可根据实际知识调整）
        label_relations = {
            "Sunny": ["Cloudy"],        # 晴天可关注多云
            "Cloudy": ["Foggy", "Rainy", "Snowy"],  # 多云可关注雾、雨、雪
            "Foggy": ["Cloudy", "Rainy"],
            "Rainy": ["Cloudy", "Snowy"],
            "Snowy": ["Cloudy", "Rainy"]
        }
        mask = torch.zeros(len(label_names), len(label_names))

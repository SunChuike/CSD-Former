# ==========================================================================================
# BACKBONE CODE (was in backbone.py)
# ==========================================================================================
...
def get_model_path(modelname, pretrain_type, pretrain_dir):
    ...
    PTDICT = {
        ...
        'swin_l_224_22k_oi':'swin_large_patch4_window7_224_22k.pth',
        'swin_l_384_22k_oi':'swin_large_patch4_window12_384_22k.pth',
    }
...
class DINOv3Backbone(nn.Module):
    """
    Wrapper for DINOv3 Vision Transformers to be used as a backbone.
    This module loads a DINOv3 model and makes it compatible with the Q2L framework.
    """
    def __init__(self, name: str, train_backbone: bool, pretrain_dir: str):
        super().__init__()
        
        if 'vitb16' in name:
            weights_file = 'dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth'
            self.num_channels = 768
            hub_fn = dinov3_vitb16
        elif 'vitl16' in name:
            weights_file = 'dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth'
            self.num_channels = 1024
            hub_fn = dinov3_vitl16
        else:
            raise ValueError(f"Unsupported DINOv3 model name: {name}")

        weights_path = os.path.join(pretrain_dir, weights_file)
        if not os.path.exists(weights_path):
            raise FileNotFoundError(f"DINOv3 weights not found at: {weights_path}")

        print(f"Loading DINOv3 weights from: {weights_path}")
        # Use the hub function to create model and load weights from the specified path
        self.body = hub_fn(pretrained=True, weights=weights_path)
    ...
    def forward(self, x: Tensor):
        # get_intermediate_layers returns a tuple of feature maps. 
        # n=1 gives the last layer. We take the first element of the tuple.
        # reshape=True ensures the output is in BxCxHxW format.
        features = self.body.get_intermediate_layers(x, n=1, reshape=True)[0]
        # The Joiner expects a dictionary output
        return {'0': features}
...
def build_backbone(args):
    ...
    # ADDED: Branch for DINOv3 models
    if args.backbone.startswith('dinov3'):
        print(f"Building DINOv3 backbone: {args.backbone}")
        backbone = DINOv3Backbone(args.backbone, train_backbone, args.pretrain_dir)
        bb_num_channels = backbone.num_channels
    elif args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_l_224_22k', 'swin_l_384_22k']:
        imgsize = int(args.backbone.split('_')[-2])
        backbone = build_swin_transformer(args.backbone, imgsize)
        if args.pretrained:
            pretrainedpath = get_model_path(args.backbone, args.pretrain_type, args.pretrain_dir)
